...
def create_google_flow():
    return google_auth_oauthlib.flow.Flow.from_client_config(
        { "web": { "client_id": os.getenv("GOOGLE_CLIENT_ID"), "client_secret": os.getenv("GOOGLE_CLIENT_SECRET"), "auth_uri": "https://accounts.google.com/o/oauth2/auth", "token_uri": "https://oauth2.googleapis.com/token", "redirect_uris": [url_for('google_callback', _external=True)], } },
        scopes=SCOPES, redirect_uri=url_for('google_callback', _external=True)
    )
...
def add_to_calendar():
    ...
    try:
        ...
        event_time = datetime.datetime.fromisoformat(concert.get('date'))
        event = { 'summary': concert.get('name'), 'location': concert.get('venue'), 'description': f"Link: {concert.get('url')}", 'start': {'dateTime': event_time.isoformat(), 'timeZone': 'Asia/Bangkok'}, 'end': {'dateTime': (event_time + datetime.timedelta(hours=2)).isoformat(), 'timeZone': 'Asia/Bangkok'}}
